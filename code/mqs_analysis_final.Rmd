...
#### Problem Description:
We have found Acupuncture treatment to have a significant effect on the Headache Severiry Score (HSS) score change with adjustment to the confounders. From the study, both the control and treatment group were given usual headache prevention medicine (Prophylactic Medication measured by MQS score), and the treatment group were given Acupunture treatments as well. Therefore, we idetify the Prophylactic Medication MQS score to be a potential Mediator or Moderator. Therefore, we shall test for any signifiance Mediation (Full or Partial). If we do not find enough evidence for mediation, we shall test for Moderation (interaction) effect.

#### Statistical Analysis:
We consider the treatment group assignment as primary predictor (X). This is referred as 'Acupunture' since it is a binary treatment group assignment. Our primary ourcome variable is the change in the HSS after 12-months, i.e., difference between 12-months and baseline (Y). The HSS at 12-months is an aggregate of everyday HSS entered for four weeks by patients after 12-months. At the same time, the patients in both control and treatment group were also taking Prophylactic medication (the usual standard medication given for prevention of headache) quantified by the MQS score (M) referred as MEDS from now onwards.
...
# Checking Moderation effect caused by M (interaction effect between X and M)
print("Checking Moderation effect caused by M (interaction effect between X and M)")
moderator_model = lm(delta5 ~ group*totalmeds5, data=data)
summary(moderator_model)
```
